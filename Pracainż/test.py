...
def createGui(portfolio, apiKey):
    ...
    #####################################
    # Część górna: Wybór kryptowaluty i wykres
    #####################################
    frame_top = ttk.Frame(scrollable_frame, padding=10)
    frame_top.pack(side="top", fill="x")

    ttk.Label(frame_top, text="Wprowadź nazwę kryptowaluty:").grid(row=0, column=0, sticky="w", padx=5, pady=5)
    cryptoVar = tk.StringVar()
    cryptoEntry = ttk.Entry(frame_top, textvariable=cryptoVar)
    cryptoEntry.grid(row=0, column=1, padx=5, pady=5)

    def showChart():
        symbol = cryptoVar.get()
        updateChart(symbol)

    ttk.Button(frame_top, text="Stwórz Wykres", command=showChart).grid(row=0, column=2, padx=5, pady=5)
    def clearChart():  # by zerowało cała  strukture canvas
        for widget in frameChart.winfo_children():
            widget.destroy()
    ttk.Button(frame_top, text="Wyczyść wykres", command=clearChart).grid(row=0, column=3, padx=5, pady=5)  # Dodano obok

    #####################################
    # Ramka wykresu (początkowo pusta)
    #####################################
    frameChartContainer = ttk.Frame(scrollable_frame)
    ...
    #ttk.Button(frameChartContainer, text="Wyczyść wykres", command=clearChart).pack(side="bottom",pady=10)  # buton widoczny i działa  odpowiednio po stronie   na ramce canvas , ma widok ale steruje czym innym i widok to canvas. (button na dole pod grafika)
    ...
    ttk.Label(frameAdd, text="Symbol kryptowaluty:").grid(row=0, column=0, sticky="w", padx=5, pady=5)
    ...
    #####################################
    # Ramki prognozy i optymalizacji (pod tabelą portfela)
    #####################################
    ...
    # symbol = cryptoVar.get() # Usunięto
        ...
        def fetchDataAndUpdateChart():
            ...
            if not prices:
                root.after(0, lambda: messagebox.showerror("Błąd", "Nie udało się pobrać danych historycznych."))
                return
            if not dates:
                root.after(0, lambda: messagebox.showerror("Błąd", "Nie udało się pobrać dat historycznych."))
                return
            ...
            if predictedMean is not None and confInt is not None:
                ...
                print("Nie udało się wygenerować prognozy.")
                ax.text(0.5, 0.5, 'Nie udało się wygenerować prognozy', horizontalalignment='center',
...
def main():
    ...
    gui= createGui(portfolio, CRYPTOCOMPARE_API_KEY)
